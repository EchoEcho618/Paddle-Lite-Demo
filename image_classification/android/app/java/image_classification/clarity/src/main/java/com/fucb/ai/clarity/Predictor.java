...
public class Predictor {
    ...
    public void postProcess() {
        ...
        if (wordLabels.size() > 0) {
            top1Result = String.format("{\"name\":\"%s\",\"score\":%.3f}", wordLabels.get(max_index[0]), max_num[0]);
            top2Result = String.format("{\"name\":\"%s\",\"score\":%.3f}", wordLabels.get(max_index[1]), max_num[1]);
        }
    }
